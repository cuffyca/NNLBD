...
class RumelhartHintonModel( BaseModel ):
    def __init__( self, print_debug_log = False, write_log_to_file = False, network_model = "hinton", model_type = "open_discovery",
        ...
        super().__init__( print_debug_log = print_debug_log, write_log_to_file = write_log_to_file, network_model = network_model, model_type = model_type,
                          ...
                          embedding_modification = embedding_modification, use_cosine_annealing = use_cosine_annealing, cosine_annealing_min = cosine_annealing_min,
    ...
    def Predict( self, primary_input_vector, secondary_input_vector ):
        ...
        with tf.device( self.device_name ):
            if self.final_layer_type in ["cosface", "arcface", "sphereface"]:
                ...
                return self.model.predict( [primary_input_vector, secondary_input_vector, outputs], verbose = self.verbose )
            else:
    ...
    def Evaluate( self, inputs_1, inputs_2, inputs_3, outputs ):
        ...
        with tf.device( self.device_name ):
            if self.final_layer_type in ["cosface", "arcface", "sphereface"]:
                loss, accuracy, precision, recall, f1_score = self.model.evaluate( [inputs_1, inputs_2, outputs], outputs, verbose = self.verbose )
            else:
                loss, accuracy, precision, recall, f1_score = self.model.evaluate( [inputs_1, inputs_2], outputs, verbose = self.verbose )
