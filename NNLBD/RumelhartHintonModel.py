...
from keras.layers import Dense, Activation, Input, Concatenate, Dropout, Embedding, Flatten, BatchNormalization, Lambda
...
class RumelhartHintonModel( BaseModel.BaseModel ):
    ...
    """
        ...
        Outputs:
            None
    """
    def Build_Model( self, number_of_features, number_of_train_1_inputs, number_of_train_2_inputs, number_of_hidden_dimensions, number_of_outputs, embeddings = [], sparse_mode = False ):
        # Update 'BaseModel' Class Variables
        ...
        if sparse_mode:
            ...
            if len( embeddings ) > 0:
                embedding_layer     = Embedding( number_of_features, number_of_hidden_dimensions, input_length = 1, name = 'Primary_Concept_Embedding_Layer', weights = [embeddings], trainable = self.trainable_weights )( primary_input_layer )
            else:
                embedding_layer     = Embedding( number_of_features, number_of_hidden_dimensions, input_length = 1, name = 'Primary_Concept_Embedding_Layer', trainable = self.trainable_weights )( primary_input_layer )
        ...
        self.model = Model( inputs = [primary_input_layer, secondary_input_layer], output = output_layer, name = self.network_model + " model" )
